...
DEFAULT_CONFIG = {
    ...
    'w_eff_threshold': 0.1,    # Minimum |Gx| to show edge
...
def select_neurons_by_degree(
    ...
    with torch.no_grad():
        ...
        if weighted:
            score = abs_Gx.sum(axis=0) + abs_Gx.sum(axis=1)
        else:
            edges = abs_Gx > threshold
            score = edges.sum(axis=0) + edges.sum(axis=1)
...
def compute_w_eff(model: nn.Module) -> np.ndarray:
    """
    Compute Gx = E @ Dx (In your model: encoder @ decoder)
    This represents the connectivity graph between sparse neurons.
    """
    with torch.no_grad():
        # In your train.py/run.py, D is n_embd, N is expansion dim
        # self.encoder: (nh, D, N)
        # self.decoder: (nh * N, D)
        
        nh = model.config.n_head
        D = model.config.n_embd
        N_per_head = D * model.config.mlp_internal_dim_multiplier // nh
        N_total = nh * N_per_head

        # Reshape encoder to be a global projection: (D, N_total)
        # We permute (nh, D, N) -> (D, nh, N) -> (D, nh*N)
        E = model.encoder.permute(1, 0, 2).reshape(D, N_total)
        
        # decoder is already (N_total, D)
        Dx = model.decoder 

        # Gx[i,j] = how much sparse neuron i affects sparse neuron j via the embedding space
        # (N_total, D) @ (D, N_total) = (N_total, N_total)
        Gx = Dx @ E 
...
def generate_neuron_animation(
    ...
    # Calculate N based on the model's actual config
    nh = model.config.n_head
    D = model.config.n_embd
    N_per_head = D * model.config.mlp_internal_dim_multiplier // nh
    N_total = nh * N_per_head
    ...
    if len(candidate_indices) == 0:
        print("  WARNING: No candidate neurons found. Try lowering w_eff_threshold.")
        return []

    # 2. Compute Gx (Connectivity)
    print("  Computing Gx (Decoder @ Encoder)...")
    Gx = compute_w_eff(model)
    ...
    edges, edge_weights, selected_indices = build_fixed_edges(
        ...
        min_component_size=cfg.get('min_component_size', 5)
    )
    
    M = len(selected_indices)
    if M == 0 or len(edges) == 0:
        print("  WARNING: Graph is empty. No edges found above threshold.")
        # We return an empty list to prevent the NoneType error in the caller
        return []

    # 4. Compute layout
    print(f"  Computing force layout for {M} neurons...")
    positions = compute_force_layout(
        edges, np.abs(edge_weights), M,
        seed=cfg['layout_seed'], iterations=150
    )

    # 5. Generate frames
    images = []
    print(f"  Generating {L} frames...")
    for layer_idx in range(L):
        fig, ax = plt.subplots(1, 1, figsize=(10, 10))

        # Average activations over tokens
        x_act = x_frames[layer_idx].mean(dim=0).cpu().numpy()[selected_indices]
        
        if layer_idx > 0:
            y_prev = y_frames[layer_idx - 1].mean(dim=0).cpu().numpy()[selected_indices]
        else:
            y_prev = np.zeros_like(x_act)

        draw_neuron_panel(ax, positions, edges, edge_weights, x_act, y_prev, layer_idx, M, N_total)
        
        plt.tight_layout()
        images.append(fig_to_pil_image(fig))

    return images # Explicitly returning the list
